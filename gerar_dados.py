...
Alimenta a Twitch Extension. Faz tudo sozinho:
  ...
  1. Abre o perfil do personagem na Armory do Warmane
  2. Captura o modelo 3D girando, em N frames (vira a animação do painel)
  3. Lê os atributos direto da página
  4. Busca o equipamento na API pública do Warmane
...
# ------------------------------------------------------------------
# 1) Equipamento, via API pública
# ------------------------------------------------------------------
def busca_api(nome, realm):
    ...
    try:
        ...
        sys.exit("A API não devolveu JSON. Confira o nome do personagem e o realm.")
    if "error" in d:
...
def captura_navegador(nome, realm, pasta, frames, debug):
    ...
    with sync_playwright() as p:
        ...
        pg = nav.new_page(viewport={"width": 1400, "height": 950})
...
def main():
    ...
    api = busca_api(args.nome, args.realm)
    stats, frames = captura_navegador(args.nome, args.realm, pasta, args.frames, args.debug)
